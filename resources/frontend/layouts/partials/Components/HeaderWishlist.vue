    ...
    <div>
        <div class="header_cart h_o_dropdown" id="wishlist" v-if="wishlistItems && wishlistItems.length > 0">
            <div class="header_cart_inner">
                <ul v-bind:style="{'height': (wishlistItems && wishlistItems.length > 2) ? '300px': 'auto'}">
                    <li v-for="(wishlistItem, cartIndex) in wishlistItems" :key="'wishlistItem_' + cartIndex">
                        ...
                        <div class="h_c_inner_left">
                            <img :src="wishlistItem.images && wishlistItem.images.length > 0 ? wishlistItem.images[0].thumbs_image_path : defultImage.value" :alt="'Hologram Product ' + wishlistItem.style_no" class="img-fluid">
                        </div>
                        <div class="h_c_inner_right">
                            <h3>
                                <router-link :to="{ name: 'single-product', params: { parent: wishlistItem ? wishlistItem.slug : '' }}">
                                    {{wishlistItem ? wishlistItem.name : 'Undefined'}}
                                </router-link>
                            </h3>
                            <p>{{wishlistItem && wishlistItem.brand ? wishlistItem.brand.name : 'Undefined'}}</p>
                            <div class="h_c_inner_info">
                                <ul>
                                    <li><span>Color</span> {{wishlistItem.color ? wishlistItem.color.name : 'Not set'}}</li>
                                    <li><span>Price</span> USD$ {{wishlistItem ? wishlistItem.price : 0 | round(2)}}</li>
                                    <li><span>Quantity</span> {{wishlistItem.quantity}}</li>
                                </ul>
                            </div>
                        </div>
                    </li>
