<template>
    <!-- ============================
        START HERO SECTION
    ============================== -->
    <cartComponent v-if="form && form.route == 'cart'"></cartComponent>
    <checkoutComponent v-else-if="form && form.route == 'checkout'"></checkoutComponent>
    <orderCompleteComponent v-else-if="form && form.route == 'order-complete'"></orderCompleteComponent>
    <productSingleComponent v-else-if="productFound === true"></productSingleComponent>
    <productListComponent v-else-if="categoryFound === true"></productListComponent>
    <div v-else-if="productFound == null || categoryFound == null" style="position:relative; height:60vh">
        <div class="preloader_wrap" id="dynamicPagePreload">
            <div class="loader-container">
                <div class="loader-circle"></div>
            </div>
        </div>
    </div>
    <div class="footer_hero_area" v-else>
        <h2 style="padding-top: 40px; text-align:center;">
            404 Not Found
        </h2>
        <h4 style="text-align:center;">
            The page you looking for, Is not active.
        </h4>
    </div>
    <!-- try product load -->
    <!-- try category load -->
    <!-- ============================
        END HERO SECTION
    ============================== -->
</template>
<script> 

    import pageStore from './pageStore'
    export default {
        name:'dynamicPage',    
        components: {
            productListComponent: () => import(/* webpackChunkName: "js/productListComponent" */ '../ecommerce/ProductList.vue'),
            productSingleComponent: () => import(/* webpackChunkName: "js/productSingleComponent" */ '../ecommerce/ProductSingle.vue'),
            cartComponent: () => import(/* webpackChunkName: "js/CartComponent" */ '../ecommerce/Cart.vue'),
            cartComponent: () => import(/* webpackChunkName: "js/CartComponent" */ '../ecommerce/Cart.vue'),
            checkoutComponent: () => import(/* webpackChunkName: "js/checkoutComponent" */ '../ecommerce/Checkout.vue'),
            orderCompleteComponent: () => import(/* webpackChunkName: "js/orderCompleteComponent" */ '../ecommerce/OrderComplete.vue'),
        },
        data(){
            return{
                productSlug: 'null',
                form: {
                    route: '', 
                    PerPage: 40, 
                    page: 1, 
                    sorting: 1, 
                    size: null, 
                    color: null, 
                    search_text:'',
                    order_by:'sorting',
                    sort_by:'asc',
                    value_id: null, 
                    brand_id: null, 
                    price: { 
                        first: null,
                        last: null, 
                    }, 
                },
            }
        },
        beforeCreate() {
            if (!(this.$store && this.$store.state && this.$store.state['pageStore'])) {
                this.$store.registerModule("pageStore", pageStore);
            }
        }, 
        mounted(){ 
            this.pagePreloads(); 
        },
        computed:{
            productFound() {  
                return this.$store.getters['pageStore/getProductFound'];
            },
            categoryFound() {  
                return this.$store.getters['pageStore/getCategoryFound'];
            },
        },
        methods:{ 
            async pagePreloads(){
                this.$Progress.start()
                this.makeRoute()
                let route = this.form.route;
                switch (route) {
                    case 'cart':
                        $("#dynamicPagePreload").fadeOut("slow");
                        break;
                    case 'checkout':
                        $("#dynamicPagePreload").fadeOut("slow");
                        break;
                    case 'order-complete':
                        $("#dynamicPagePreload").fadeOut("slow");
                        break;
                    default:
                        const currentQuery = this.$route.query;
                        this.form = this.$route.query.filters ? JSON.parse(this.$route.query.filters) : {
                            route: '', 
                            PerPage: 40, 
                            page: 1, 
                            sorting: 1, 
                            size: null, 
                            color: null, 
                            search_text:'',
                            order_by:'sorting',
                            sort_by:'asc',
                            value_id: null, 
                            brand_id: null, 
                            price: { 
                                first: null,
                                last: null, 
                            }, 
                        };  
                        this.makeRoute()
                        await this.$store.dispatch('pageStore/singleProduct',  this.productSlug); 
                        await this.$store.dispatch('pageStore/products',  this.form)
                        .then((response)=>{ 
                        })
                        .catch((error) => {
                        })
                        $("#dynamicPagePreload").fadeOut("slow");
                        break;
                }
            },
            makeRoute(){
                this.form.search_text = this.$route.query.s
                if(this.$route.params.third){ 
                    this.form.route = this.$route.params.third
                    this.productSlug = null;
                    return;
                }
                if(this.$route.params.sub){ 
                    this.form.route = this.$route.params.sub
                    this.productSlug = null;
                    return;
                }
                if(this.$route.params.parent){ 
                    this.form.route = this.$route.params.parent
                    this.productSlug = this.$route.params.parent;
                    return;
                } 
            },
        },
    }
</script>