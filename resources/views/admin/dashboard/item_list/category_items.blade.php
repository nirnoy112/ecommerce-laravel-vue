        ...
        <div class="accordion_body default_accrodion open" id="ActiveItems">
            <div class="list_item_content">
                <div class="pagination_wrapper p10 pt_0">
                    ...
                    <ul class="pagination">
                        <li><button class="ly_btn p1_first{{ $activeItemsArray->currentPage() > 1 ?  ' btn_paginate' : ''}}"{{ $activeItemsArray->currentPage() == 1 ?  ' disabled' : ''}}>| <i class="fas fa-chevron-left"></i></button></li>
                        <li>
                            <button class="ly_btn p1_prev{{ $activeItemsArray->currentPage() > 1 ?  ' btn_paginate' : ''}}"{{ $activeItemsArray->currentPage() == 1 ?  ' disabled' : ''}}> <i class="fas fa-chevron-left"></i> PREV</button>
                        </li>
                        <li>
                            <div class="pagination_input">
                                <input type="number" min="1" max="{{ $activeItemsArray->lastPage() }}" class="form_global p1" value="{{ $activeItemsArray->currentPage() }}"> of {{ $activeItemsArray->lastPage() }}
                            </div>
                            <div class="pagination_btn">
                                <button class="ly_btn switch_page">GO</button>
                            </div>
                        </li>
                        <li><button class="ly_btn p1_next{{ $activeItemsArray->currentPage() < $activeItemsArray->lastPage() ?  ' btn_paginate' : ''}}"{{ $activeItemsArray->currentPage() == $activeItemsArray->lastPage() ?  ' disabled' : ''}}>  NEXT <i class="fas fa-chevron-right"></i></button></li>
                        <li>
                            <button class="ly_btn p1_last{{ $activeItemsArray->currentPage() < $activeItemsArray->lastPage() ?  ' btn_paginate' : ''}}"{{ $activeItemsArray->currentPage() == $activeItemsArray->lastPage() ?  ' disabled' : ''}}> <i class="fas fa-chevron-right"></i> |</button>
                        </li>
                    </ul>
                </div>
                <hr>
                    ...
                    @foreach($activeItems as $item)                
                        <div class="item_list">
                            <div class="custom_checkbox">
                                <input class="checkbox-active-items" type="checkbox" id="productCB_{{ $item->item_id }}" data-id="{{ $item->item_id }}">
                                <label for="productCB_{{ $item->item_id }}"></label>
                            </div>
                            <div class="item_list_text">
                                <span class="single_img">
                                    <a href="{{ route('admin_edit_item', ['item' => $item->item_id]) }}">
                                    ...
                                    <a href="{{ route('admin_edit_item', ['item' => $item->item_id]) }}" onclick="centeredmodal(this.href,'myWindow','1150','800','yes');return false" class="edit"><i class="fas fa-edit"></i></a>
                                </span>
                                <span class="item_list_desc">
                                    <h2>{{ $categoryName->name }}</h2>
                                    <h3><a href="{{ route('admin_edit_item', ['item' => $item->item_id]) }}">{{ $item->style_no }}</a></h3>
                                    <h2>
    ...
    <div class="ly_accrodion inactive_item_list">
        <div class="ly_accrodion_heading display_table">
            <div class="ly_accrodion_title accordion_heading open_acc" data-toggle="accordion" data-target="#InactiveItems" data-class="accordion">
                <span>  Inactive Items - {{ sizeof($inactiveItems) }} Items</span>
            </div>
        </div>
        <div class="accordion_body default_accrodion open" id="InactiveItems">
            <div class="list_item_content">
                <div class="pagination_wrapper p10 pt_0">
                    ...
                    <ul class="pagination">
                        <li><button class="ly_btn p2_first{{ $inactiveItemsArray->currentPage() > 1 ?  ' btn_paginate' : ''}}"{{ $inactiveItemsArray->currentPage() == 1 ?  ' disabled' : ''}}>| <i class="fas fa-chevron-left"></i></button></li>
                        <li>
                            <button class="ly_btn p2_prev{{ $inactiveItemsArray->currentPage() > 1 ?  ' btn_paginate' : ''}}"{{ $inactiveItemsArray->currentPage() == 1 ?  ' disabled' : ''}}> <i class="fas fa-chevron-left"></i> PREV</button>
                        </li>
                        <li>
                            <div class="pagination_input">
                                <input type="number" min="1" max="{{ $inactiveItemsArray->lastPage() }}" class="form_global p2" value="{{ $inactiveItemsArray->currentPage() }}"> of {{ $inactiveItemsArray->lastPage() }}
                            </div>
                            <div class="pagination_btn">
                                <button class="ly_btn switch_page">GO</button>
                            </div>
                        </li>
                        <li><button class="ly_btn p2_next{{ $inactiveItemsArray->currentPage() < $inactiveItemsArray->lastPage() ?  ' btn_paginate' : ''}}"{{ $inactiveItemsArray->currentPage() == $inactiveItemsArray->lastPage() ?  ' disabled' : ''}}>  NEXT <i class="fas fa-chevron-right"></i></button></li>
                        <li>
                            <button class="ly_btn p2_last{{ $inactiveItemsArray->currentPage() < $inactiveItemsArray->lastPage() ?  ' btn_paginate' : ''}}"{{ $inactiveItemsArray->currentPage() == $inactiveItemsArray->lastPage() ?  ' disabled' : ''}}> <i class="fas fa-chevron-right"></i> |</button>
                        </li>
                    </ul>
                </div>
                <hr>
                ...
                <div class="item_list_wrapper p10">
                    @foreach($inactiveItems as $item)    
                        <div class="item_list">
                            <div class="custom_checkbox">
                                <input class="checkbox-inactive-items" type="checkbox" id="productCB_{{ $item->item_id }}" data-id="{{ $item->item_id }}">
                                <label for="productCB_{{ $item->item_id }}"></label>
                            </div>
                            <div class="item_list_text">
                                <span class="single_img">
    ...
    <script>
        $(function () {
            ...
            $('.p1_prev').click(function() {
                var p1 = <?php echo $activeItemsArray->currentPage(); ?> - 1;
                var p2 = $('.p2').val();
                var currentLocation = String(window.location);
                var switchPageUrl = currentLocation.split('?')[0] + '?p1=' + p1 + '&p2=' + p2;
            ...
            $('.p1_last').click(function() {
                var p1 = <?php echo $activeItemsArray->lastPage(); ?>;
                var p2 = $('.p2').val();
                var currentLocation = String(window.location);
                var switchPageUrl = currentLocation.split('?')[0] + '?p1=' + p1 + '&p2=' + p2;
            ...
            $('.p2_prev').click(function() {
                ...
                var p2 = <?php echo $inactiveItemsArray->currentPage(); ?> - 1;
                var p1 = $('.p1').val();
                var currentLocation = String(window.location);
                var switchPageUrl = currentLocation.split('?')[0] + '?p1=' + p1 + '&p2=' + p2;
            ...
            $('.p2_last').click(function() {
                ...
                var p2 = <?php echo $inactiveItemsArray->lastPage(); ?>;
                var p1 = $('.p1').val();
                var currentLocation = String(window.location);
                var switchPageUrl = currentLocation.split('?')[0] + '?p1=' + p1 + '&p2=' + p2;
            ...
            $(document).on('click','.btnCloneActive', function() {
                var ids = [];
