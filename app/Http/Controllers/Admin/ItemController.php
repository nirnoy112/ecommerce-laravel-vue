...
class ItemController extends Controller
{
    ...
    public function createNewItemPost(Request $request) {
        ...
        if ($request->imagesId) {
            for ($i = 0; $i < sizeof($request->imagesId); $i++) {
                ...
                $allPaths = imageMove($image->image_path, $folderName = 'items', $imageStoreTypes = ['image_path', 'compressed_image_path', 'thumbs_image_path']);
    ...
    public function uploadImage(Request $request) {
        ...
        $image->fullPath = asset($temporaryPath);
    ...
    public function itemListAll(Request $request) {
        ...
        return view('admin.dashboard.item_list.index', compact( 'activeItems', 'inactiveItems', 'appends',
        ...
        if ($request->imagesId) {
            for ($i = 0; $i < sizeof($request->imagesId); $i++) {
                ...
                if (is_null($tmp->item_id)) {
                    ...
                    $allPaths = imageMove($tmp->image_path, $folderName = 'items', $imageStoreTypes = ['image_path', 'compressed_image_path', 'thumbs_image_path']);

                    $tmp->item_id = $item->id;
                    $tmp->sort = $i + 1;
                    $tmp->image_path = $allPaths['image_path'];
                    $tmp->compressed_image_path = $allPaths['compressed_image_path'];
                    $tmp->thumbs_image_path = $allPaths['thumbs_image_path'];
                    $tmp->save();
                } else {
    ...
    public function itemListByCategory(Category $category, Request $request) {
        ...
        $activeItemsArray = DB::table('items')
        ...
        $activeItemsArray = $activeItemsArray->where(function($q) use($category) {
            $q->where('item_categories.default_parent_category', $category->id)
                ->orwhere('item_categories.default_second_category', $category->id)
                ->orwhere('item_categories.default_third_category', $category->id);
        });

        //Active Item search
        if ($request->text){
            $data = explode(',', $request->text);
            $activeItemsArray = $activeItemsArray->where(function ( $q) use ($data, $request){
                if (isset($request->style) && $request->style == '1') {
                    foreach ($data as $value){
                        $q->orWhere('style_no', 'like', '%' . ltrim($value, ' ') . '%');
                    }
        ...
        if (isset($request->s1) && $request->s1 != '') {
            if ($request->s1 == '4')
                $activeItemsArray = $activeItemsArray->orderby('items.price', 'asc');
            else if ($request->s1 == '1')
                $activeItemsArray = $activeItemsArray->orderby('items.updated_at', 'desc');
            else if ($request->s1 == '2')
                $activeItemsArray = $activeItemsArray->orderby('items.created_at', 'desc');
            else if ($request->s1 == '3')
                $activeItemsArray = $activeItemsArray->orderby('items.activated_at', 'desc');
            else if ($request->s1 == '5')
                $activeItemsArray = $activeItemsArray->orderby('items.price', 'desc');
            else if ($request->s1 == '6')
                $activeItemsArray = $activeItemsArray->orderby('items.style_no');
            else if ($request->s1 == '0') {
                $activeItemsArray = $activeItemsArray->orderby('items.activated_at', 'desc');
            }
        } else {
        ...
        $activeItemsArray = $activeItemsArray->paginate(50, ['*'], 'p1');

        $itemsWithImagesArray = array();

        foreach($activeItemsArray as $key=>$ai) {
            $itemsWithImages = $ai;
            $active_item_id = $ai->item_id;
            $images_sql_query = "SELECT `item_images`.* FROM `item_images` WHERE `item_images`.item_id = $active_item_id ORDER BY `item_images`.sort";

            $itemsWithImages->images = DB::select($images_sql_query);
            array_push($itemsWithImagesArray, $itemsWithImages);

        }

        $activeItems = $itemsWithImagesArray;

        $inactiveItemsArray = DB::table('items')
            ->join('item_categories', 'items.id', '=', 'item_categories.item_id')
            ->select('items.*', 'item_categories.item_id')
        ...
        $inactiveItemsArray = $inactiveItemsArray->where(function($q) use($category) {
            $q->where('item_categories.default_parent_category', $category->id)
                ->orwhere('item_categories.default_second_category', $category->id)
                ->orwhere('item_categories.default_third_category', $category->id);
        });
        ...
        if ($request->text){
            $data = explode(',', $request->text);
            $inactiveItemsArray = $inactiveItemsArray->where(function ( $q) use ($data, $request){
                if (isset($request->style) && $request->style == '1') {
                    foreach ($data as $value){
                        $q->orWhere('style_no', 'like', '%' . ltrim($value, ' ') . '%');
                    }
        ...
        //Inactive Item Sorting
        if (isset($request->s2) && $request->s2 != '') {
            if ($request->s2 == '4')
                $inactiveItemsArray = $inactiveItemsArray->orderby('items.price', 'asc');
            else if ($request->s2 == '1')
                $inactiveItemsArray = $inactiveItemsArray->orderby('items.updated_at', 'desc');
            else if ($request->s2 == '2')
                $inactiveItemsArray = $inactiveItemsArray->orderby('items.created_at', 'desc');
            else if ($request->s2 == '3')
                $inactiveItemsArray = $inactiveItemsArray->orderby('items.activated_at', 'desc');
            else if ($request->s2 == '5')
                $inactiveItemsArray = $inactiveItemsArray->orderby('items.price', 'desc');
            else if ($request->s2 == '6')
                $inactiveItemsArray = $inactiveItemsArray->orderby('items.style_no');
            else if ($request->s2 == '0') {
                $inactiveItemsArray = $inactiveItemsArray->orderby('items.activated_at', 'desc');
            }
        } else {
        ...
        $inactiveItemsArray = $inactiveItemsArray->paginate(50, ['*'], 'p2');

        $itemsWithImagesArray = array();
        
        foreach($inactiveItemsArray as $key=>$ai) {
            $itemsWithImages = $ai;
            $inactive_item_id = $ai->item_id;
            $images_sql_query = "SELECT `item_images`.* FROM `item_images` WHERE `item_images`.item_id = $inactive_item_id ORDER BY `item_images`.sort";
        ...
        $inactiveItems = $itemsWithImagesArray;

        $appends = [
            'p1' => $activeItemsArray->currentPage(),
            'p2' => $inactiveItemsArray->currentPage(),
        ];
        ...
        return view('admin.dashboard.item_list.category_items', compact('activeItemsArray', 'activeItems', 'inactiveItemsArray', 'inactiveItems',
             'appends', 'vendor', 'defaultCategories', 'vendorCategories', 'categoryName','categories'))
